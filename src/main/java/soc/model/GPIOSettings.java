...
public class GPIOSettings {
	 ...
	 *
	 * @param commandPin
	 *            the pin used tell the DE1 to encrypt or decrypt the data
	 * @param piActivePin
	 *            the pin used to tell the DE1 the Pi is sending
	 * @param de1ActivePin
	 *            the pin used to tell the Pi the DE1 is sending
	 * @param piActiveWaitTime
	 *            time the Pi waits after setting the piActivePin
	 */
	public GPIOSettings(int commandPinNr, int piActivePinNr,
			int de1ActivePinNr, long piActiveWaitTime) {
	...
	public GPIOSettings(GPIOSettings settings) {
		this(settings.commandPinNr, settings.piActivePinNr,
				settings.de1ActivePinNr, settings.piActiveWaitTime);
	}
	...
	/**
	 * Gets the pin used tell the DE1 whether to encrypt or decrypt the data.<br>
	 * On the first usage the pin will be created.
	 *
	 * @param gpio
	 *            the gpio object used
	 * @return the pin
	 */
	...
	public GpioPinDigitalInput getDe1ActivePin(GpioController gpio) {
		if (de1ActivePin == null)
			try {
				de1ActivePin = gpio.provisionDigitalInputPin((Pin) getPinField(
						de1ActivePinNr).get(null));
	 ...
	 * @return the pin
	 */
	private synchronized final GpioPinDigitalOutput getOutPin(
			GpioController gpio, int pinNr) {
		try {
			return gpio.provisionDigitalOutputPin((Pin) getPinField(pinNr).get(
					null));
		} catch (IllegalArgumentException | IllegalAccessException e) {
			return null;
		}
	 ...
	 *            the number of the pin
	 * @param setting
	 *            the default pin settings (high or low)
	 * @return the pin
	 */
	@SuppressWarnings("unused")
	private synchronized final GpioPinDigitalOutput getOutPin(
			GpioController gpio, int pinNr, PinState setting) {
		try {
			return gpio.provisionDigitalOutputPin(
					(Pin) getPinField(pinNr).get(null), setting);
		} catch (IllegalArgumentException | IllegalAccessException e) {
			return null;
		}
	}
	 ...
	 */
	@SuppressWarnings("unused")
	private synchronized final GpioPinDigitalInput getInPin(
			GpioController gpio, int pinNr) {
		try {
			return gpio.provisionDigitalInputPin((Pin) getPinField(pinNr).get(
					null));
		} catch (IllegalArgumentException | IllegalAccessException e) {
			return null;
		}
	 ...
	 *            the number of the pin
	 * @param setting
	 *            the default pin setting
	 * @return the pin
	 */
	@SuppressWarnings("unused")
	private synchronized final GpioPinDigitalInput getInPin(
			GpioController gpio, int pinNr, PinPullResistance setting) {
		try {
	...
	/**
	 * Gets the static field within the RaspiPin class which represents this
	 * pin.
	 *
	 * @param pinNr
	 *            the number of the pin
	...
	private synchronized Field getPinField(int pinNr) {
		try {
			return RaspiPin.class.getDeclaredField("GPIO_"
					+ (pinNr < 10 ? "0" : "") + pinNr);
