...
/**
 * Represents an entire file which can be send to the DE1 or is received from
 * the DE1.<br>
 * The size will be a multiple of 128 bits.
 *
 * @author rvemous
 *
 */
public class Message {
	...
	/**
	 * Default size in bytes of one packet.<br>
	 * The size of a message is allowed to be be 16, 32, 64 or 128 bytes.
	 */
	 ...
	 *            the data to use
	 * @param actualSize
	 *            the actual size of the data from index 0, useful when the data
	 *            has already been padded with random data
	 * @param encrypted
	 *            whether the data is encrypted data or plain text
	 * @param packetSize
	 *            the packet size to use, when it the data length is unequal to
	 *            a multiple of this value it will be padded with
	...
	public Message(byte[] data, int actualSize, boolean encrypted,
		...
		if (data.length % packetSize != 0) {
			...
			System.arraycopy(randomBuff, 0, messageData, data.length,
					randomBuff.length);
		} else
			messageData = data;
	}
	...
	/**
	 * Creates a new message fit to be send to the DE1 for encryption/decryption
	 * with the default packet size.<br>
	 *
	 * @param data
	 *            the data to use
	 * @param actualSize
	...
	/**
	 * Creates a new message fit to be send to the DE1 for encryption/decryption
	 * where <code>data</code> is unpadded.<br>
	 *
	 * @param data
	 *            the data to use
	 * @param encrypted
	...
	/**
	 * Creates a new message fit to be send to the DE1 for encryption/decryption
	 * with the default packet size and where <code>data</code> is unpadded.<br>
	 *
	 * @param data
	 *            the data to use
	 * @param encrypted
	...
	/**
	 * Creates a new message fit to be send to the DE1 for encryption/decryption
	 * with the default packet size, where <code>data</code> is unpadded and
	 * unencrypted.<br>
	 *
	 * @param data
	 *            the data to use
	 ...
	 * Should only be used when it is actually encrypted or decrypted by the
	 * DE1.
	 *
	 * @param encrypted
	...
	public String toString() {
		return "Message - encrypted: " + encrypted + ", actual size: "
				+ actualSize + ", data: " + Arrays.toString(messageData);
	}
	 ...
	 *
	 * @param compact
	 *            if true only writes out the length of the data
	 */
	public String toString(boolean compact) {
		if (compact)
			return "Message - encrypted: " + encrypted + ", actual size: "
			+ actualSize + ", data: " + messageData.length + " bytes";
		else
			return toString();
	}
