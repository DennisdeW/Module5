...
public class SocManagerTest {
	 ...
	 * @param selfTest whether to test the Pi sending to itself 
	 * (MOSI and MISO ports connected) or to the DE1
	 */
	public static void testSocManager(int sendSpeed, int packetSize, int fileSize, int bufferSize, int timeOut, boolean selfTest) {
		// set up manager
		...
		Assert.assertEquals("Data length is not a multiple of " + packetSize + " bytes",
				0, msg.getData().length % packetSize); 
