...
public class SocManagerTest {
	...
	public void testSocManager() {
		...
		Assert.assertEquals("Data length is not a multiple of 16 bytes", 0,
				msg.getData().length % 16);
		...
		Assert.assertNotNull(
				"Encryption response from DE1 timed out (>1000ms)", encrypted);
		Assert.assertFalse("Encryption not done in time (>2000ms)",
				timer.hasExpired());
		timer.restart();
		Message decrypted = manager.sendAndReceiveData(encrypted, false, 1000);
		Assert.assertNotNull(
				"Decryption response from DE1 timed out (>1000ms)", decrypted);
		Assert.assertFalse("Decryption not done in time (>2000ms)",
				timer.hasExpired());
